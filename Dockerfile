...
RUN git clone https://github.com/ass-a2s/SoniTranslate.git
...
RUN /opt/conda/bin/conda run -n sonitr conda env config vars set YOUR_HF_TOKEN="$GET_YOUR_HF_TOKEN"

# Set up environment variables in conda
RUN /opt/conda/bin/conda run -n sonitr conda env config vars set OPENAI_API_KEY="$GET_OPENAI_API_KEY"
